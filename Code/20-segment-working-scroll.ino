...
//Scrolles through serial messages
...
const byte numChars = 141;  //MAX ammount of chars that can be input (+1 for terminating character)
...
const uint8_t digitSegments[][5] = {
  ...
  //{ 0b00000011, 0b11110000, 0b00000000, 0b01111111, 0b11111000 }, /*T (uppercase)*/
  { 0b00001100, 0b01101111, 0b10111101, 0b11110000, 0b00000000 }, /*t (lowercase)*/
...
const uint32_t digitColors[12] = {
  pixels.Color(255, 0, 0), //Panel 1
  pixels.Color(245, 10, 0), //Panel 2
  pixels.Color(235, 20, 0), //Panel 3
  pixels.Color(225, 30, 0), //Panel 4
  pixels.Color(215, 40, 0), //Panel 5
  pixels.Color(205, 50, 0), //Panel 6
  pixels.Color(195, 60, 0), //Panel 7
  pixels.Color(185, 70, 0), //Panel 8
  pixels.Color(175, 80, 0), //Panel 9
  pixels.Color(165, 90, 0), //Panel 10
  pixels.Color(0, 225, 30),
  pixels.Color(55, 200, 0)
};
...
*/
int speed = 150; //delay - how fast do you want it to scroll?
int frames = 0;
...
void loop() {
  receiveData();
  showNewData();
  for(int j=9;j>((frames*-1)-1);j--){ //j = how many characters after end of string
    determineDigit(j);
...
void determineDigit(int place) {
  for (int panNum = 0; panNum <=frames; panNum++) { //where the message starts
    if (strlen(myData) > 0) {
    ...
    for (int byteIndex = 0; byteIndex < 5; byteIndex++) {
      ...
      for (int bitIndex = 0; bitIndex < 8; bitIndex++) {
        ...
        pixelIndex = pixelIndex + ((place + panNum) * 38);

        bool bitValue = (byteValue >> (7 - bitIndex)) & 1;  // Extract the bit value (1 or 0)

        // If the bit is 1, turn the pixel on; if it's 0, turn it off
        if (bitValue) {
          pixels.setPixelColor(pixelIndex, digitColors[10]); //set to whatever color you want, as defined above
        } else {
